...
def extract_ligand(complex_pdb: str, output_pdb: str = "LIG_final.pdb", resid: str = "LIG") -> str:
    logger.info(f"Extracting ligand {resid} from complex...")
    run_cmd(f"grep '^HETATM' {complex_pdb} | grep ' {resid} ' > {output_pdb}", shell=True)
    return output_pdb


def prepare_ligand(ligand_pdb: str, charge: int = 0):
    logger.info("Protonating ligand...")
    run_cmd(["obabel", "-ipdb", ligand_pdb, "-opdb", "-O", "LIG_H.pdb", "-h"])

    logger.info("Running antechamber (SYBYL cleanup)...")
    run_cmd([ 
        "antechamber", "-i", "LIG_final_H.pdb", "-fi", "pdb", "-o", "LIG_sybl.mol2",
        "-fo", "mol2", "-j", "5", "-at", "sybyl", "-dr", "no", "-s", "2"
        ])
    
    logger.info("Running antechamber (GAFF2 + charges)...")
    run_cmd([
        "antechamber", "-i", "LIG_sybl.mol2", "-fi", "mol2", "-o", "LIG.mol2", "-fo", "mol2",
        "-at", "gaff2", "-c", "bcc", "-s", "2", "-rn", "LIG", "-nc", str(charge)
        ])
    
    logger.info("Running parmchk2...")
    run_cmd(["parmchk2", "-i", "LIG.mol2", "-f", "mol2", "-o", "LIG.frcmod"])

    return "LIG.mol2", "LIG.frcmod"
...
def main():
    ...
    complex_file = str(args.complex.resolve())
