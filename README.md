...
```bash
python prepare_protein_ligand_complex.py \
  --input_pdb "INPUT_COMPLEX.pdb" \
  --ligand_resname "LIG" \
  --ligand_charge 0 \
  --verbose
```

If RESP charges required, `utils/resp.py` can be used. This script assigns RESP charges using Psi4.
